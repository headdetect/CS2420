...
public class AVLTree
{
	 ...
	 * 
	 * @param cur
	 *            : The node to check the balance for, usually you start with the parent of a leaf.
	 */
	...
	public void remove(int k)
	{
		...
		removeFoundNode(find(this.root, k));
	}
	...
	{
		return find(root, i.intValue()) != null;
	}

	private AvlNode find(AvlNode p, int q)
	{
		if (p == null)
		{
			return null;
		...
		else
		{
			if (p.key > q)
			{
				return find(p.left, q);
			}
			else if (p.key < q)
			{
				return find(p.right, q);
			}
			else
			{
				return p;
...
class AvlNode
{
	...
	public homework7.Data.Node<String> asOtherNode(homework7.Data.Node<String> parent)
	{
		...
		homework7.Data.Node<String> node = new homework7.Data.Node<String>(key + "");
